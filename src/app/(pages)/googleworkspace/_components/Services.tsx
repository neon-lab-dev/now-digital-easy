...
const ServiceCard: React.FC<ServiceCardProps> = ({ title, description }) => {
    ...
    return (
        <div className='flex justify-center  z-50 p-4 my-4 max-lg:my-0 '>
            <div className={`flex flex-col w-[368px] h-[200px] bg-[#F0F1FF] p-8  gap-2 rounded-xl relative overflow-hidden shadow-lg `}
                ...
                <span className={` text-[24px] font-source-sans-pro font-900  ${isHovered ? 'text-white' : 'text-[#0437CD]'} ${isHovered ? 'z-30' : 'z-1'}`}>{title}</span>
                <span className={` text-[15px] font-merriweather font-400 ${isHovered ? 'text-white' : 'text-[#646464]'} ${isHovered ? 'z-30' : 'z-1'}`}>{description}</span>
            </div>
        </div>
    );
};
